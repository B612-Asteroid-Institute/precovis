...
def plotIntercectingHealpixels(plot, objectName, startDate, endDate, dur, observatory="I41", dataset_id="ztf"):    
    ...
    #Healpixel Set-up  
    obs_frames = frames[(frames["obscode"] == observatory) & (frames["dataset_id"] == dataset_id)]
    obs_frames_sorted = obs_frames.sort_values("exposure_mjd_mid")
    merged_healpixels = pd.merge_asof(obs_frames_sorted, coords_df, left_on="exposure_mjd_mid", right_on="time", by="healpixel", tolerance=dur).dropna() #by="healpixel"
...
def plotSetUp(title, height=400, width=400):
